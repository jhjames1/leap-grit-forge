...
const PeerPerformanceDashboard = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        {metrics.map(specialist => <Card key={specialist.peer_id}>
            <CardHeader>
                    ...
                    <TooltipTrigger asChild>
                      <div className="space-y-2">
                        ...
                        <div className={`text-xl font-bold ${getMetricColor(specialist.avg_streak_impact, 1)} flex items-center gap-1`}>
                          {(specialist.avg_streak_impact || 0) >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
               ...
               {/* Performance Alerts & Coaching for Individual Specialist */}
                <div className="mt-4 pt-4 border-t">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {(specialist.chat_completion_rate || 0) < 75 && <div className="text-xs bg-muted/50 rounded p-2">
                          <strong>Chat Completion:</strong> Focus on consistent session engagement and follow-through
                        </div>}
                      
                      {(specialist.checkin_completion_rate || 0) < 75 && <div className="text-xs bg-muted/50 rounded p-2">
                          <strong>Check-ins:</strong> Improve scheduling adherence and time management
                        </div>}
                      
                      {(specialist.avg_response_time_seconds || 0) > 45 && <div className="text-xs bg-muted/50 rounded p-2">
                          <strong>Response Time:</strong> Practice quicker responses and use templates
                        </div>}
                      
                      {(specialist.avg_user_rating || 0) < 4.5 && <div className="text-xs bg-muted/50 rounded p-2">
                          <strong>User Satisfaction:</strong> Focus on active listening and empathy
                        </div>}
                      
                      {(specialist.avg_streak_impact || 0) < 1 && <div className="text-xs bg-muted/50 rounded p-2">
                          <strong>Streak Impact:</strong> Incorporate goal-setting techniques
                        </div>}
                      
                      {(specialist.chat_completion_rate || 0) >= 75 && (specialist.checkin_completion_rate || 0) >= 75 && (specialist.avg_response_time_seconds || 0) <= 45 && (specialist.avg_user_rating || 0) >= 4.5 && (specialist.avg_streak_impact || 0) >= 1 && <div className="text-xs bg-muted/50 rounded p-2">
