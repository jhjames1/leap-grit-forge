    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-xs text-green-500">
                  {specialistAnalytics.totalSpecialists > 0 
                    ? Math.round((specialistAnalytics.activeSpecialists / specialistAnalytics.totalSpecialists) * 100)
                    : 0}% availability
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`w-3 h-3 ${
                        star <= specialistAnalytics.averageRating
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className={`text-xs ${
                  specialistAnalytics.averageResponseTime < 60 ? 'text-green-500' :
                  specialistAnalytics.averageResponseTime < 300 ? 'text-yellow-500' : 'text-red-500'
                }`}>
                  {specialistAnalytics.averageResponseTime < 60 ? 'Excellent' :
                   specialistAnalytics.averageResponseTime < 300 ? 'Good' : 'Needs improvement'}
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="md:col-span-2">
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              {specialistAnalytics.alertFlags.length === 0 ? (
                <div className="text-sm text-green-500 text-center py-2">
                  ✓ No active alerts - All specialists performing well
                </div>
              ) : (
                specialistAnalytics.alertFlags.slice(0, 3).map((alert, index) => (
              ...
              {specialistAnalytics.alertFlags.length > 3 && (
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-lg font-semibold mb-4">Active Specialists</div>
          <div className="space-y-4">
            {specialistAnalytics.specialistPerformance.map((specialist) => (
              <div key={specialist.specialistId} className="flex items-center justify-between p-4 border rounded-lg">
            ...
            {specialistAnalytics.specialistPerformance.length === 0 && (
