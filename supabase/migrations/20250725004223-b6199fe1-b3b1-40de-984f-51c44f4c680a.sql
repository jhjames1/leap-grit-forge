...
BEGIN
  ...
  LOOP
    ...
    AND scheduled_start > current_timestamp
    AND scheduled_start <= current_timestamp + interval '15 minutes';
    ...
    ELSE
      ...
      IF working_hours IS NOT NULL THEN
        ...
        day_name := lower(to_char(current_timestamp, 'Day'));
        ...
        IF (working_hours -> day_name ->> 'enabled')::boolean = true THEN
          ...
          current_time_only := current_timestamp::TIME;
    ...
    WHERE specialist_status.updated_at < current_timestamp - interval '30 minutes'
